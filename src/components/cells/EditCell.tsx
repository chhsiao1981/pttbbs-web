...
import type { EditRune_t } from "../../types";
...
export default (props: Props) => {
  ...
  const text = rune.isTail ? "" : rune.text;

  // console.log('EditCell: rune.isTail:', rune.isTail, 'rune.text:', rune.text || '', 'text:', text)

  const [classNames0] = getClassNamesFromRune(rune);
  if (rune.pullright) {
  ...
  const onKeyDownInput = (e: KeyboardEvent) => {
    ...
    const valueCount = calcTextCount(focusInputRef.current.value);
    const keyCount = e.key.length === 1 ? calcTextCount(e.key) : 0;
    ...
    if (e.nativeEvent.isComposing) {
      switch (e.key) {
        case "Enter":
          console.log(
            "EditCell.onKeyDownInput: to set Enter: value:",
  ...
  const onKeyUpInput = (e: KeyboardEvent) => {
    ...
    const textCount = calcTextCount(focusInputRef.current.value);
  ...
  const onFocusInput = (_e: FocusEvent) => {
    //console.log('EditCell.onFocusInput: start: e:', e.target)
  };

  const onBlurInput = (_e: FocusEvent) => {
