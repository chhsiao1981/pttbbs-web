...
export const getClassNamesFromRune = (rune: any): [string[], boolean] => {
  const classNames0 = [styles.rune];
  const color0 = rune.color0 || {};
  classNames0.push(...getClassNamesFromColor(color0));
  const isTwoColor =
    rune.color1 &&
    _runeAttrs.some((attr) => color0[attr] !== rune.color1[attr]);
  if (isTwoColor) {
...
export const calcTextCount = (text: string): number => {
  return text.split("").reduce((r, x, _i) => {
    if (x >= " " && x <= "~") {
      return r + 1;
    }
    return r + 2;
